    ...
    describe('Making Orders', () => {
        ...
        describe('Failure', () => {
            it('Rejects order that has no balance', async () => {
                transaction = await expect(exchange.connect(user1).makeOrder(token2.address, tokens(1), token1.address, tokens(1))).to.be.reverted;
            })
        })
    })
